...
class SVD_LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, ratio=1, ranks=None):
        ...
        def _rank(name, default, dim):
            ...
            r = max(1, min(r, dim))
            return r
        q_rank = _rank("q_proj", low_rank, self.hidden_size)
        k_rank = _rank("k_proj", low_rank, self.num_key_value_heads * self.head_dim)
        v_rank = _rank("v_proj", low_rank, self.num_key_value_heads * self.head_dim)
        o_rank = _rank("o_proj", low_rank, self.hidden_size)
    ...
    def forward(
        ...
        if self.num_key_value_groups > 1:
            key_states = key_states[:, :, None, :, :].expand(
                bsz, self.num_key_value_heads, self.num_key_value_groups, kv_seq_len, self.head_dim
            ).reshape(bsz, self.num_heads, kv_seq_len, self.head_dim)
